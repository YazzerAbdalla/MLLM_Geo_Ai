        ...
        try:
            ...
            for i, cell_id in enumerate(cell_ids):
                ...
                if text_desc:
                    ...
                    word_counts = Counter(text_desc.split())
                    poi_top = [w for w, _ in word_counts.most_common(3)]

                results.append({
                    "cell_id": cell_id,
                    "dominant_class": classes[int(np.argmax(p))],
                    "confidences": {
                    ...
                    "text_embedding_norm": float(
                        np.linalg.norm(
                            self.poi_encoder.embed_texts([text_desc])[0]
                            if text_desc else np.zeros(384)
                        )
                    ),
                    "graph_embedding_norm": float(np.linalg.norm(np.array([
                        node_count,
                        total_length,
                        cell_info['avg_degree']
                    ], dtype=np.float32))),
                    ...
                    "satellite_thumbnail_url":
                        f"/api/v1/thumbnails/{grid_id}/{cell_id}.jpg"
                })
